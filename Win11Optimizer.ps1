# Win11Optimizer.ps1 - Updated with Multi-Selection for Windows Updates and Fixed Syntax Errors
...
$lblWelcome.Location = New-Object System.Drawing.Point(10, 20)
$lblWelcome.Size = New-Object System.Drawing.Size(540, 50)
$lblWelcome.Text = "Welcome to Win11 Optimizer"
...
$lblSubtitle.Location = New-Object System.Drawing.Point(10, 70)
$lblSubtitle.Size = New-Object System.Drawing.Size(540, 30)
$lblSubtitle.Text = "Optimize and manage your Windows 11 experience"
$lblSubtitle.ForeColor = [System.Drawing.Color]::FromArgb(200, 200, 200)
$lblSubtitle.Font = New-Object System.Drawing.Font("Segoe UI", 10)
$lblSubtitle.TextAlign = "MiddleCenter"
$containerSystemInfo.Controls.Add($lblSubtitle)

$panelSysInfo = New-Object System.Windows.Forms.Panel
$panelSysInfo.Location = New-Object System.Drawing.Point(10, 110)
$panelSysInfo.Size = New-Object System.Drawing.Size(540, 500)
...
$sysInfoText.Size = New-Object System.Drawing.Size(510, 480)
...
$listUpdates.SelectionMode = "MultiSimple"
...
$btnInstallSecurity.Add_Click({
    $selectedUpdates = $listUpdates.SelectedItems
    if ($selectedUpdates.Count -gt 0) {
        $totalUpdates = $selectedUpdates.Count
        $progressBarUpdates.Visible = $true
        $progressBarUpdates.Maximum = $totalUpdates
        $progressBarUpdates.Value = 0
        $currentUpdate = 0
        ...
        foreach ($update in $selectedUpdates) {
            $currentUpdate++
            $kb = $update -replace '\[KB(\d+)\].*', '$1'
            $lblStatusUpdates.Text = "Status: Installing update $currentUpdate of $totalUpdates (KB$kb)..."
            Install-WindowsUpdate -MicrosoftUpdate -KBArticleID $kb -AcceptAll -IgnoreReboot -ErrorAction SilentlyContinue
            $progressBarUpdates.Value = $currentUpdate
            $form.Refresh()
        }
                    ...
                    try {
                        if ($command -match '^"([^"]+)"' -or $command -match '^([^\s]+)') {
                            ...
                            if (Test-Path $exePath) {
                                ...
                                $publisher = if ((Get-ItemProperty $exePath -ErrorAction SilentlyContinue).VersionInfo.CompanyName) { 
                                    (Get-ItemProperty $exePath).VersionInfo.CompanyName 
                                } else { 
                                    "Unknown" 
                                }
                                $filePath = $exePath
