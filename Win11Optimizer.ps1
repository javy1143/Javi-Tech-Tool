...
function Ensure-PSWindowsUpdate {
    if (-not (Get-Module -ListAvailable -Name PSWindowsUpdate)) {
        $lblStatusUpdates.Text = "Installing PSWindowsUpdate module..."
        try {
            Install-Module -Name PSWindowsUpdate -Force -Scope CurrentUser -Confirm:$false -ErrorAction Stop
        } catch {
            $lblStatusUpdates.Text = "Failed to install PSWindowsUpdate: $_"
            return $false
        }
    }
...
function Check-RestartRequired {
    try {
        if ((Get-WURebootStatus).RebootRequired) {
            [System.Windows.Forms.MessageBox]::Show("A restart is required to complete the update(s).", "Restart Required")
        }
    } catch {
...
$btnLoadSecurity.Add_Click({
    ...
    if ($script:securityUpdates) {
        $total = $script:securityUpdates.Count
        $progressBarUpdates.Visible = $true
        $progressBarUpdates.Maximum = $total
        $progressBarUpdates.Value = 0

        $i = 0
        foreach ($u in $script:securityUpdates) {
            $i++
            $lblStatusUpdates.Text = "Status: Loading $i of $total..."
            $listUpdates.Items.Add("[KB$($u.KB)] $($u.Title)")
            $progressBarUpdates.Value = $i
            $form.Refresh()
        }
        $lblStatusUpdates.Text = "Status: $total security updates loaded"
    } else {
...
$btnInstallSecurity.Add_Click({
    ...
    foreach ($u in $script:securityUpdates) {
        $i++
        $lblStatusUpdates.Text = "Installing $i of $total (KB$($u.KB))..."
        try {
            Install-WindowsUpdate -KBArticleID $u.KB -AcceptAll -IgnoreReboot -ErrorAction Stop
            $listUpdates.Items.Add("[KB$($u.KB)] $($u.Title) - ✅ Installed")
        } catch {
            $listUpdates.Items.Add("[KB$($u.KB)] $($u.Title) - ❌ Failed")
        }
        $progressBarUpdates.Value = $i
        $form.Refresh()
    }
    ...
    $lblStatusUpdates.Text = "Status: Security updates installed"
    $progressBarUpdates.Visible = $false
    $script:securityUpdates = $null
})
...
$btnLoadAll.Add_Click({
    ...
    if ($script:allUpdates) {
        $total = $script:allUpdates.Count
        $progressBarUpdates.Visible = $true
        $progressBarUpdates.Maximum = $total
        $progressBarUpdates.Value = 0

        $i = 0
        foreach ($u in $script:allUpdates) {
            $i++
            $lblStatusUpdates.Text = "Status: Loading $i of $total..."
            $listUpdates.Items.Add("[KB$($u.KB)] $($u.Title)")
            $progressBarUpdates.Value = $i
        ...
        $lblStatusUpdates.Text = "Status: $total updates loaded"
    } else {
...
$btnInstallAll.Add_Click({
    ...
    Check-RestartRequired
    $lblStatusUpdates.Text = "Status: All updates installed"
    $progressBarUpdates.Visible = $false
    $script:allUpdates = $null
})
