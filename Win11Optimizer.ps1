...
# Create form with larger default size
...
function Load-Apps {
    ...
    if ($wingetAvailable) {
        ...
        # Merge winget apps, avoiding duplicates
...
$btnUninstall.Add_Click({
    ...
    Load-Apps # Refresh the list after uninstall
})
...
# Define the list of apps with their download URLs
$appList = @(
    [PSCustomObject]@{ Name = "7-Zip"; Url = "https://www.7-zip.org/a/7z2301-x64.exe" }, # Keeping 7-Zip from the previous list
    [PSCustomObject]@{ Name = "RingCentral"; Url = "https://app.ringcentral.com/download/RingCentral.exe?V=20138739841159500" },
    [PSCustomObject]@{ Name = "Google Chrome"; Url = "https://dl.google.com/chrome/install/ChromeSetup.exe" }, # Direct download link for Chrome
...
# Populate the checklist with app names
    ...
    foreach ($appName in $selectedApps) {
        ...
        if ($app) {
            ...
            try {
                ...
                # Run the installer silently (assumes .exe; adjust for .msi if needed)
...
# Button: Install Security Updates
