...
$tabStartup.BackColor = $form.BackColor

$clbStartup = New-Object System.Windows.Forms.CheckedListBox
$clbStartup.Width = 540
$clbStartup.Height = 300
$clbStartup.Top = 10
$clbStartup.Left = 10
...
$lblStartupStatus.Top = 360
$lblStartupStatus.Left = 10
$lblStartupStatus.Font = $clbStartup.Font
$lblStartupStatus.ForeColor = $clbStartup.ForeColor
$lblStartupStatus.Text = "Status: Ready"

$btnApplyStartupChanges = New-Object System.Windows.Forms.Button
$btnApplyStartupChanges.Text = "Apply Changes"
$btnApplyStartupChanges.Width = 140
$btnApplyStartupChanges.Height = 32
$btnApplyStartupChanges.Top = 320
$btnApplyStartupChanges.Left = 160
$btnApplyStartupChanges.Font = $clbStartup.Font
$btnApplyStartupChanges.BackColor = [System.Drawing.Color]::FromArgb(0, 94, 184)
$btnApplyStartupChanges.ForeColor = [System.Drawing.Color]::White
$btnApplyStartupChanges.FlatStyle = "Flat"
$btnApplyStartupChanges.FlatAppearance.BorderSize = 0
            ...
            if (Test-Path $reg.Path) {
                ...
                foreach ($prop in $items.PSObject.Properties) {
                    ...
                    if ($value.Length -ge 1) {
                        $enabled = ($value[0] -eq 2)
                        $approvedApps[$prop.Name] = $enabled
                    }
                }
            }
        }
        ...
        if ($reg.Path -notlike "*StartupApproved*") {
            if (Test-Path $reg.Path) {
                ...
                foreach ($name in $items.PSObject.Properties.Name) {
                    $enabled = $true
                    if ($approvedApps.ContainsKey($name)) {
                        $enabled = $approvedApps[$name]
                    }
    ...
    foreach ($reg in $registryPaths) {
        if (Test-Path $reg.Path) {
            ...
            foreach ($name in $items.PSObject.Properties.Name) {
                ...
                $clbStartup.Items.Add($name, $true) | Out-Null
            }
        }
    }
...
$tabStartup.Controls.Add($lblStartupStatus)
$tabStartup.Controls.Add($clbStartup)
$tabStartup.Controls.Add($btnApplyStartupChanges)
