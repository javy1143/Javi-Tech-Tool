...
$tabInstall.BackColor = $form.BackColor
$tabInstall.ForeColor = $form.ForeColor

$tabSpeedUp = New-Object System.Windows.Forms.TabPage
$tabSpeedUp.Text = "Speed Up Windows"
$tabSpeedUp.BackColor = $form.BackColor
$tabSpeedUp.ForeColor = $form.ForeColor

$tabCustom = New-Object System.Windows.Forms.TabPage
$tabCustom.Text = "Custom Preferences"
$tabCustom.BackColor = $form.BackColor
$tabCustom.ForeColor = $form.ForeColor

$tabUpdates = New-Object System.Windows.Forms.TabPage
$tabUpdates.Text = "Windows Updates"
$tabUpdates.BackColor = $form.BackColor
$tabUpdates.ForeColor = $form.ForeColor

$tabUninstall = New-Object System.Windows.Forms.TabPage
$tabUninstall.Text = "Uninstall"
$tabUninstall.BackColor = $form.BackColor
$tabUninstall.ForeColor = $form.ForeColor
...
# -------------------------
# Tab: Custom Preferences Controls
...
$tabUpdates.Controls.Add($btnCheckSecUpdates)
...
$tabUpdates.Controls.Add($btnInstallSecUpdates)
...
$tabUpdates.Controls.Add($btnCheckAllUpdates)
...
$tabUpdates.Controls.Add($btnInstallAllUpdates)

# -------------------------
# Tab: Uninstall Controls

# Load Installed Software Button
$btnLoadSoftware = New-Object System.Windows.Forms.Button
$btnLoadSoftware.Location = New-Object System.Drawing.Point(20,20)
$btnLoadSoftware.Size = New-Object System.Drawing.Size(200,40)
$btnLoadSoftware.Text = "Load Installed Software"
$btnLoadSoftware.FlatStyle = "Flat"
$btnLoadSoftware.BackColor = $buttonBackColor
$btnLoadSoftware.ForeColor = $buttonForeColor
$btnLoadSoftware.Add_Click({
    try {
        $installedSoftware = Get-InstalledSoftware
        if ($installedSoftware -and $installedSoftware.Count -gt 0) {
            # Create a DataTable with typed columns
            $dataTable = New-Object System.Data.DataTable "InstalledSoftware"
            [void]$dataTable.Columns.Add("DisplayName",[System.Type]::GetType("System.String"))
            [void]$dataTable.Columns.Add("UninstallString",[System.Type]::GetType("System.String"))
            
            # Populate the DataTable with rows, skipping invalid items
            foreach ($app in $installedSoftware) {
                if ($app -and $app.DisplayName -and $app.UninstallString) {
                    try {
                        $row = $dataTable.NewRow()
                        $row["DisplayName"] = [string]$app.DisplayName
                        $row["UninstallString"] = [string]$app.UninstallString
                        $dataTable.Rows.Add($row)
                    }
                    catch {
                        Log-Message "Skipping an item due to error: $($_.Exception.Message)"
                    }
                }
                else {
                    Log-Message "Skipping item missing DisplayName or UninstallString."
                }
            }
            
            # Bind the DataTable to the DataGridView
            $dgvUninstall.DataSource = $null
            $dgvUninstall.DataSource = $dataTable
            $dgvUninstall.AutoGenerateColumns = $true
            $dgvUninstall.AutoSizeColumnsMode = [System.Windows.Forms.DataGridViewAutoSizeColumnsMode]::AllCells
            $dgvUninstall.Refresh()

            # Hide the UninstallString column if present
            if ($dgvUninstall.Columns.Contains("UninstallString")) {
                $dgvUninstall.Columns["UninstallString"].Visible = $false
            }
            # Add an unbound checkbox column "Select" as the first column if not already present
            if (-not $dgvUninstall.Columns.Contains("Select")) {
                $cbCol = New-Object System.Windows.Forms.DataGridViewCheckBoxColumn
                $cbCol.Name = "Select"
                $cbCol.HeaderText = "Select"
                $cbCol.Width = 50
                $cbCol.ReadOnly = $false
                $dgvUninstall.Columns.Insert(0, $cbCol)
            }
            # Set the data-bound columns as read-only
            if ($dgvUninstall.Columns.Contains("DisplayName")) {
                $dgvUninstall.Columns["DisplayName"].ReadOnly = $true
            }
            if ($dgvUninstall.Columns.Contains("UninstallString")) {
                $dgvUninstall.Columns["UninstallString"].ReadOnly = $true
            }
            
            Log-Message "Installed software list loaded. ($($installedSoftware.Count) items)"
        }
        else {
            Log-Message "No installed software found."
        }
    }
    catch {
        [System.Windows.Forms.MessageBox]::Show("Error loading software list: $($_.Exception.Message)")
    }
})
$tabUninstall.Controls.Add($btnLoadSoftware)

# Create the DataGridView for displaying installed software
$dgvUninstall = New-Object System.Windows.Forms.DataGridView
$dgvUninstall.Location = New-Object System.Drawing.Point(20,80)
$dgvUninstall.Size = New-Object System.Drawing.Size(800,400)
$dgvUninstall.SelectionMode = "FullRowSelect"
$dgvUninstall.MultiSelect = $true
$dgvUninstall.AutoGenerateColumns = $true
# Allow editing so the checkbox column is selectable
$dgvUninstall.ReadOnly = $false

# Set the grid appearance for a dark theme
$dgvUninstall.BackgroundColor = $tabUninstall.BackColor
$dgvUninstall.ForeColor = $tabUninstall.ForeColor
$dgvUninstall.DefaultCellStyle.BackColor = $tabUninstall.BackColor
$dgvUninstall.DefaultCellStyle.ForeColor = $tabUninstall.ForeColor
$dgvUninstall.DefaultCellStyle.SelectionBackColor = [System.Drawing.Color]::FromArgb(80,80,80)
$dgvUninstall.DefaultCellStyle.SelectionForeColor = $tabUninstall.ForeColor
$dgvUninstall.ColumnHeadersDefaultCellStyle.BackColor = $buttonBackColor
$dgvUninstall.ColumnHeadersDefaultCellStyle.ForeColor = $buttonForeColor
...
$btnUninstallSelected.Location = New-Object System.Drawing.Point(240,20)
$btnUninstallSelected.Size = New-Object System.Drawing.Size(200,40)
$btnUninstallSelected.Text = "Uninstall Selected"
$btnUninstallSelected.FlatStyle = "Flat"
$btnUninstallSelected.BackColor = $buttonBackColor
$btnUninstallSelected.ForeColor = $buttonForeColor
$btnUninstallSelected.Add_Click({
    $rowsToUninstall = @()
    foreach ($row in $dgvUninstall.Rows) {
        if ($row.Cells["Select"].Value -eq $true) {
            $rowsToUninstall += $row
        }
    }
    if ($rowsToUninstall.Count -eq 0) {
        [System.Windows.Forms.MessageBox]::Show("No applications selected for uninstallation.")
        return
    }
    foreach ($row in $rowsToUninstall) {
        $drv = $row.DataBoundItem  # DataRowView
        $displayName = $drv["DisplayName"]
        $uninstallString = $drv["UninstallString"]

        Log-Message "Attempting to uninstall: $displayName"
        try {
            Start-Process -FilePath "cmd.exe" -ArgumentList "/c $uninstallString" -Wait -NoNewWindow
            Log-Message "$displayName uninstalled (if command executed successfully)."
        }
        catch {
            Log-Message "Failed to uninstall ${displayName}: $($_.Exception.Message)"
        }
    }
})
...
# -------------------------
# Add a log window (RichTextBox) at the bottom of the form
$global:logBox = New-Object System.Windows.Forms.RichTextBox
$global:logBox.Multiline = $true
$global:logBox.ScrollBars = "Vertical"
$global:logBox.Location = New-Object System.Drawing.Point(20,610)
$global:logBox.Size = New-Object System.Drawing.Size(850,60)
$global:logBox.ReadOnly = $true
$global:logBox.BackColor = [System.Drawing.Color]::FromArgb(30,30,30)
$global:logBox.ForeColor = [System.Drawing.Color]::White
